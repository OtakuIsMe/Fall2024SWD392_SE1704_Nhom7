...
const RoomDetail = () => {
  ...
  const [typeNumberServiceSelected, setTypeNumberServiceSelected] =
    useState("0");
  ...
  const totalPrice = (
    ...
    const total =
      Number(room) * Number(totalTimeInHour(start, end)) +
      Number(service) +
      services;
  ...
  const fetchServices = async (): Promise<void> => {
    try {
      ...
      response.forEach((row: any, index: number) => {
        const serviceData = createData(
          index,
          row.amenityService.id,
  ...
  useEffect(() => {
    ...
    setSelectedItemList([]);
    const date = new Date();
    const curTime = dayjs(date)
      .set("minute", 0)
      .add(1, "hour")
      .format("YYYY-MM-DDThh:mm");
    const maxTime = dayjs(date)
      .set("hour", 0)
      .set("minute", 0)
      .add(30, "day")
      .format("YYYY-MM-DDThh:mm");
  ...
  useEffect(() => {
    const total = selectedItemList.reduce(
      (total, item) => total + item.service.price * item.amount,
      0
    );
  ...
  const handleAddItem = (service: ServiceData) => {
    ...
    setSelectedItemList((prev) => {
      const existingItem = prev.find(
        (item) => item.service.index === service.index
      );
      if (existingItem) {
  ...
  return (
    <div id="room-detail-page">
        ...
        <div className="room-detail-container">
          <div className="room-info">
            ...
            <div className="nav-bar">
              <div
                className="nav info-container"
                onClick={() => {
                  handleNavbarClick("info-container");
                }}
              >
                Infomation
              </div>
              <div
                ...
                onClick={() => {
                  handleNavbarClick("schedule");
                }}
              >
                Book Arrangement
              </div>
              <div
                className="nav comments"
                onClick={() => {
                  handleNavbarClick("comments");
                }}
              >
                Feedback
              </div>
            ...
            {infoSelected === "comments" && (
              <React.Fragment>
                <div className="info-comments">There are no comments</div>
              </React.Fragment>
            )}
          </div>
          <form className="room-booking" onSubmit={postBookingRoom}>
            <p className="price-booking">
              {priceConvert(roomInfo?.price)}VND/h
            </p>
            <div className="booking-detail">
              ...
              <div className="hour-booking date">
                ...
                <span>
                  {" "}
                  {totalTime(startDate, endDate)} <FaRegClock />
                </span>
              </div>
            ...
            <div className="total">
              ...
              <div className="Room-price line">
                <span className="title-result">Room:</span>
                <span className="results">
                  {priceConvert(roomInfo?.price)} X{" "}
                  {totalTimeInHour(startDate, endDate)}h
                </span>
              </div>
              <div className="services-price line">
                <span className="title-result">Services:</span>
                <span className="results">{priceConvert(totalService)}</span>
              </div>
              <div className="membsership-price line">
                <span className="title-result">Membership:</span>
                <span className="results">0%</span>
              </div>
              <div className="total-price line">
                <span className="title-result">Total Price:</span>
                <span className="results">
                  {totalPrice(
                    roomInfo?.price,
                    "0",
            ...
            <div
              className="service-btn but"
              onClick={async () => {
                await fetchServices(), setOpenPopup(true);
              }}
            >
              Additional Services
            </div>
            <div className="but" onClick={() => handleOpenMsgPopup()}>
              Request To Book
            </div>
            <div
              className="service-popup"
              style={!openPopupMsg ? { display: "none" } : { display: "flex" }}
            >
              <div className="noti">
                <svg
      ...
      <div
        className="service-popup"
        style={!openPopup ? { display: "none" } : { display: "flex" }}
      >
        <div className="service-board">
          <div className="title-close">
            <div>
              <p className="title">Additional Services</p>
              <p className="short-title">
                Enhance your experience with our curated range of premium
        ...
        <div className="service-popup">
          <div className="caution">
            <ErrorOutlineOutlinedIcon
              sx={{ fontSize: "48px", color: "#F12425" }}
            />
            <p>You haven't logged in yet!</p>
            <div className="btns">
              <div
                className="cancel"
                onClick={() => setOpenCautionPopup(false)}
              >
                Cancel
              </div>
